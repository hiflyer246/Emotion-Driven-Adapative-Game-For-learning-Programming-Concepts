...
# Just configure here, services use their own model instances or we can create a global one
global_model = genai.GenerativeModel(os.getenv("GEMINI_MODEL", "gemini-2.0-flash-exp"))
...
questions_db = []
# sessions = {} # logic moved to DB in interview router, but this might be for code runner questions?
# The code runner sessions are separate. Let's keep them if they are used by other routers.
sessions = {}
...
async def startup_event():
    ...
    if len(questions_db) == 0:
        print("🔄 Database empty. Generate questions using POST /api/generate-questions or run scripts/populate_questions.py")
...
async def register(user: UserCreate):
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail="Database connection failed. Ensure MongoDB is running."
        )
